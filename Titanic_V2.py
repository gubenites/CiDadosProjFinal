...
# ...e base de teste
test = (full >> mask(X.Train == 0)).copy()

test = test.sort_values(by='PassengerId').drop(['Survived'], axis = 1) 
test_survived = pd.read_csv(results_file).sort_values(by='PassengerId').drop('PassengerId', axis = 1) 
...
################### HIPER PARAMETROS TUNING
#### TUNING REG LOG
model_reglog = models_list[0]
...
grid_search = GridSearchCV(estimator=model_reglog, param_grid=grid, n_jobs=-1, cv=cv, scoring='accuracy',error_score=0)
grid_result = grid_search.fit(cp_train, survived)


#### TUNING RANDOMFOREST
model_RFC = models_list[1]
...
rf_grid = RandomizedSearchCV(estimator = model_RFC, param_distributions = grid, n_iter = 100, cv = 4, verbose = 2, random_state = 1, n_jobs = -1)
...
cross_validation_table=pd.DataFrame({'Modelo_Nome':[model_RFC,model_reglog],
               'Melhor_Acerto':[rf_grid.best_score_,grid_result.best_score_],
               'Paramentros':[rf_grid.best_params_,grid_result.best_params_],
               'Modelo':[rf_grid,grid_result] })

best_model=cross_validation_table[cross_validation_table['Melhor_Acerto']==max(cross_validation_table['Melhor_Acerto'])].reset_index(drop=True).to_dict('records')[0]

print('Modelo:',best_model['Modelo_Nome'])
print("Melhor acerto: {} usando {}".format(round(best_model['Melhor_Acerto'],4), best_model['Paramentros']))




# É feito o predict
model=best_model['Modelo']
...
clas_report = classification_report(list(test_survived), predict)
cm = confusion_matrix(list(test_survived),predict)
...
ax.imshow(cm)
ax.grid(False)
ax.xaxis.set(ticks=(0, 1), ticklabels=('Predição de Vítimas', 'Predição de Sobreviventes'))
ax.yaxis.set(ticks=(0, 1), ticklabels=('Realidade de Vítimas', 'Realidade de Sobreviventes'))
ax.set_ylim(1.5, -0.5)
for i in range(2):
    for j in range(2):
...
result = permutation_importance(
    model, test, test_survived, n_repeats=10, random_state=42, n_jobs=2
)

sorted_importances_idx = result.importances_mean.argsort()
importances = pd.DataFrame(
    result.importances[sorted_importances_idx].T,
    columns=test.columns[sorted_importances_idx],
)
